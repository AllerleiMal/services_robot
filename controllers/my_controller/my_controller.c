...
int main(int argc, char **argv) {
  ...
  wb_camera_enable(camera, wb_robot_get_basic_time_step());
  wb_camera_recognition_enable(camera, wb_robot_get_basic_time_step());
  ...
  while (true) {
    ...
    for (int i = 0; i < number_of_objects ; i++) {
        double* colors = objects[i].colors;
        if (colors != NULL && current_color[0] == colors[0] && 
          current_color[1] == colors[1] && current_color[2] == colors[2]) {
            if (objects[i].size[0] > 0.1) {
              printf("In front of the house\n");
              blink_leds();
            }else{
               printf("In front\n");
            }
    ...
    while (wb_receiver_get_queue_length(receiver) > 0) {
      ...
      if (!strcmp(message, "red")) {
        ...
        current_number = atoi(message);
      }
      printf("current color = %lf %lf %lf\n", 
